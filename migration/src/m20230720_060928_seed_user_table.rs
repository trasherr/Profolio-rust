...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        let me = user::ActiveModel { 
            ...
            phone: Set("9685741230".to_owned()),
            phone_code: ActiveValue::set(91),
            experience: Set(rand::thread_rng().gen_range(0..20)),
            ctc: Set(rand::thread_rng().gen_range(0..20)),
            company: Set(Some("Litehires".to_owned())),
            profession: Set(Some("Software Developer".to_owned())),

            ..Default::default()
        };
        me.insert(db).await.unwrap();

        let tester = user::ActiveModel { 
            name: Set("Prakhar Tester".to_owned()),
            uuid: Set(Uuid::new_v4()),
            email: Set("howtoprakhar@gmail.com".to_owned()) ,
            password: Set(create_hash("12345678", Sha256::default())),
            phone: Set("9898653258".to_owned()),
            phone_code: ActiveValue::set(91),
            experience: Set(rand::thread_rng().gen_range(0..20)),
            ctc: Set(rand::thread_rng().gen_range(0..20)),
            company: Set(Some("Litehires".to_owned())),
            profession: Set(Some("Software tester".to_owned())),

            ..Default::default()
        };
        tester.insert(db).await.unwrap();
